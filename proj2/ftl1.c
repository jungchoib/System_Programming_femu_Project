...
static void print_lpn_access_distribution(struct ssd *ssd) {
    FILE *output_file = fopen("lpn_access_distribution.csv", "w");
    if (!output_file) {
        ftl_err("Failed to open output file for LPN distribution\n");
        return;
    }

    fprintf(output_file, "LPN,Access_Count\n");
    for (uint64_t lpn = 0; lpn < ssd->sp.tt_pgs; lpn++) {
    ...
    fclose(output_file);
    ftl_log("LPN access distribution written to lpn_access_distribution.csv\n");
}

// FEMU 종료 시 호출
void femu_exit(struct ssd *ssd) {
...
// FEMU 종료 예상 시간 추적
        ...
        if (elapsed_time >= 300) { // FIO runtime (300초) 종료 시점 감지
            ftl_log("Predicted FEMU exit after 300 seconds\n");
            femu_exit(ssd); // femu_exit 호출
            break;
        }
